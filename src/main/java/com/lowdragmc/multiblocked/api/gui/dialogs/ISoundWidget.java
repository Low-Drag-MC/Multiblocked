...
public class ISoundWidget extends DialogWidget {
    ...
    private String getName(SoundEvent soundEvent) {
        if (isRemote()) {
            WeighedSoundEvents eventAccessor = Minecraft.getInstance().getSoundManager().getSoundEvent(soundEvent.getLocation());
            if (eventAccessor != null) {
                Component textComponent = eventAccessor.getSubtitle();
