...
public abstract class MultiCapabilityTrait extends CapabilityTrait {
    ...
    public void serialize(@Nullable JsonElement jsonElement) {
        ...
        for (JsonElement element : jsonArray) {
            ...
            slotName[i] = GsonHelper.getAsString(jsonObject, "slotName", "");
            x[i] = GsonHelper.getAsInt(jsonObject, "x", 5);
            y[i] = GsonHelper.getAsInt(jsonObject, "y", 5);
            autoIO[i] = GsonHelper.getAsBoolean(jsonObject, "autoIO", false);
            i++;
        }
    }
    ...
    protected void initSettingDialog(DialogWidget dialog, DraggableWidgetGroup slot, final int index) {
        ...
        dialog.addWidget(new SelectorWidget(100, 50, 65, 15, Arrays.asList("multiblocked.gui.trait.auto_io", "multiblocked.gui.trait.passive_io"), -1)
                ...
                .setHoverTooltips("multiblocked.gui.trait.auto"));
        ...
        dialog.addWidget(new TextFieldWidget(100, 10, 65, 15, null, s -> slotName[index] = s)
    ...
    protected IO[] getRealMbdIO() {
        var realIO = new IO[mbdIO.length];
